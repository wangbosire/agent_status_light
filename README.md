...
AgentStatusLight 将一个普通的红绿灯挂件改造成可由电脑控制的桌面状态灯。
...
agent_status_light send --mode busy --source codex --session my-session --ttl 1800
...
```bash
agent_status_light install [codex/cursor/claude]
agent_status_light install [codex/cursor/claude] --dir .
```

Windows：

```powershell
agent_status_light.exe install [codex/cursor/claude]
agent_status_light.exe install [codex/cursor/claude] --dir .
```

安装逻辑会合并已有 JSON 配置，并在重复安装时先移除旧的 AgentStatusLight Hook 条目，避免重复触发。
...
`--session auto` 会从 Hook stdin JSON 中优先提取 `session_id` / `sessionId` / `conversation_id` / `thread_id` / `tabId` 等会话字段；如果目标工具没有提供会话 ID，则使用 `cwd` / `workspace_path` / `transcript_path` 等稳定上下文字段生成哈希作为兜底 session。
...
状态会在 daemon 内保留一段时间；高优先级状态过期后，会自动回落到其它仍然活跃的低优先级状态。手动执行 `agent_status_light send --mode off` 会清空所有 source/session；Hook 中的 `off` 只清除对应 source/session。
...
```bash
agent_status_light uninstall [codex/cursor/claude]
agent_status_light uninstall [codex/cursor/claude] --dir .
```

卸载只会移除命令中包含 `agent_status_light send --mode` 的 Hook 条目，不会删除用户手写的其它 Hook。
