...
```text
<global_config_dir>/runtime/events.jsonl
```
...
`.agent-status-light/` 只保存 AgentStatusLight 的二进制副本和安装清单。
...
建议通过 Rust `directories` crate 获取，不手写平台路径。

参考路径：

```text
macOS:   ~/Library/Application Support/agent-status-light/
Windows: %APPDATA%/agent-status-light/
Linux:   ~/.config/agent-status-light/
```
...
项目级安装不创建 daemon runtime。daemon runtime 始终使用用户全局目录。

### 5.3 运行时目录

daemon 是用户级服务，不属于单个项目。因此运行时目录始终位于全局配置目录：

```text
<global_config_dir>/runtime/
├─ daemon.pid
├─ daemon.log
├─ events.jsonl
├─ ipc.json
...
```text
<global_config_dir>/runtime/token
```
...
```bash
.agent-status-light/bin/agent_status_light send --mode busy --source codex --session auto --ttl 1800 --quiet --hook-id agent-status-light
```
...
```text
./.codex/hooks.json
./.agent-status-light/bin/
./.agent-status-light/temp/
